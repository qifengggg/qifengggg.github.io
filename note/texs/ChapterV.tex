...
\sssubsection{形如 $ y^{(n)} = f(x) $ 时}

只需要积分 $ n $ 次即可得到方程通解。

\sssubsection{不显含函数$ y $ 的二阶可降阶方程 $ y^\pprime = f(x,y')\rightarrow $ 缺 $ y $  }

令 $ y = p'(x) $ ，则有 $ y^\pprime = \dfrac{\mathrm{d} p}{\mathrm{d}x}  =p' $ ，因而
可以将原方程降为一阶微分方程 $ p' = f(x,p). $ 

\sssubsection{不显含自变量 $ x $ 的二阶可降阶方程 $  y^\pprime = f(y,y')\rightarrow $ 缺 $ x $ }
...
考虑二阶线性微分方程$ y^\pprime + p(x)y' + q(x)y = f(x), $ 其有如下结构，也即通解。
